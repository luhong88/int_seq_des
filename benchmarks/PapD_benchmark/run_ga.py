...
# Perform multistate PapD complex design with NSGA-II
...
protein= Protein(
    ...
    chains_neighbors_list= [['A', 'B'], ['C', 'D'], ['E', 'F']],
...
if batch_settings_dict[batch_ind]['objective_type'] == 'af2rank':
    metrics_list= [
        config.af2rank_chainAB(), 
        config.af2rank_chainCD(), 
        config.af2rank_chainEF()
    ]
    observer_metrics_list= [
        config.esm_1v(),
        config.protein_mpnn_chainAB(), 
        config.protein_mpnn_chainCD(), 
        config.protein_mpnn_chainEF()
    ]
